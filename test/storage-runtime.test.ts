...
describe("storage runtime", () => {
  it("loads the built-in sqlite provider and prepares storage", async () => {
    const databasePath = join(await mkdtemp(join(tmpdir(), "gitlab-agentic-webhooks-storage-runtime-")), "storage.sqlite");
    ...
    try {
      ...
      expect(runtime.preparation.appliedMigrationIds).toEqual(["sqlite:0001_v0_baseline"]);
    } finally {
  ...
  it("rejects incompatible provider contracts", async () => {
    const providerModule = join(process.cwd(), "test", "fixtures", "storage", "incompatible-provider.ts");
  ...
  it("fails startup when provider preparation fails", async () => {
    const providerModule = join(process.cwd(), "test", "fixtures", "storage", "failing-provider.ts");
