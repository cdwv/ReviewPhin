...
>(fragments: Registry, files: Files): PromptTemplateDefinition {
  return definePromptTemplate(() =>
    renderPromptSections(fragments, files).join("\n\n"),
  );
}
...
>(fragments: Registry, file: K, params: FragmentParams<Registry, K>): string {
  const definition = fragments[file] as PromptFragmentDefinition<
    FragmentParams<Registry, K>
  >;
...
>(templates: Registry, id: K, params: TemplateParams<Registry, K>): string {
  const definition = templates[id] as PromptTemplateDefinition<
    TemplateParams<Registry, K>
  >;
...
function renderPromptSections<
  Registry extends FragmentRegistry,
  const Files extends readonly ParamlessFragmentFile<Registry>[],
>(fragments: Registry, files: Files): string[] {
...
>(fragments: Registry, file: K): string {
  return renderPromptFragment(
    fragments,
    file,
    NO_PROMPT_PARAMS as FragmentParams<Registry, K>,
  );
}
