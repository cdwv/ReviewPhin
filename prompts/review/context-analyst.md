...
Treat `changedFiles` as the complete review boundary. When Git inspection is available, call `git_readonly` at least once and use it to read focused diffs and history from the trusted base/head revisions instead of expecting every patch body in the prompt.
