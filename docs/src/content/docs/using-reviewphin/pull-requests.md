...
Up to 10 referenced images and 25 MiB of image data are included in one run. Each image also has a 10 MiB limit. If an image is unsupported, exceeds a limit, or cannot be downloaded, the run continues with the available text and images. The model is told which referenced images were unavailable so it does not claim to have inspected them.
