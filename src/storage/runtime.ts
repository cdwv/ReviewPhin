...
import { createStorageHelpers, type StorageHelpers } from "./storage-helpers.js";
import type { StorageProvider, StorageProviderFactoryContext, StorageProviderModule } from "./provider.js";
...
export async function initializeStorageRuntime(input: {
  ...
  try {
    ...
    if (preparation.storageContractRevision !== CURRENT_STORAGE_CONTRACT_REVISION) {
      throw new Error(
...
async function loadStorageProviderFactory(
...
): Promise<(context: StorageProviderFactoryContext) => Promise<StorageProvider> | StorageProvider> {
  const loadedModule = (await import(moduleSpecifier)) as Partial<StorageProviderModule> & {
    default?: unknown;
  };
  const candidate =
  ...
  if (typeof candidate !== "function") {
    throw new Error(`Storage provider module ${moduleSpecifier} must export createStorageProvider(context)`);
  }

  return candidate as (context: StorageProviderFactoryContext) => Promise<StorageProvider> | StorageProvider;
}
